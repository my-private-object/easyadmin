$(document).ready(function() {
	$.ajaxSetup({
		...
		complete : function(XMLHttpRequest, textStatus) {
			...
			if (sessionstatus == "portalorgtimeout") {
				// 前台 机构 如果超时就处理 ，指定要跳转的页面
				window.location.replace(getContextPath() + "/portalOrgLogin/tologin.do");
			} else if (sessionstatus == "portaltimeout") {
				// 前台 如果超时就处理 ，指定要跳转的页面
				window.location.replace(getContextPath() + "/portal/login/index.do");
			} else if (sessionstatus == "managetimeout") {
				// 后台 如果超时就处理 ，指定要跳转的页面
				window.location.replace(getContextPath() + "/manage/login/login.jsp");
			}
		}
	});
